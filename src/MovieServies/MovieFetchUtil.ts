...
export function makeOptions(
  ...
  if (addToken) {
    // ...ts-ignore ignorer typescript når vi har behov for det. "lint" er et andet 'program' der kan virke invasivt.
    //@ts-ignore
...
export async function handleHttpErrors(res: Response) {
  if (!res.ok) {
    const errorResponse = await res.json(); // lav to kald for at se om der er fejl fra min json!
